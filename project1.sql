...
and brand campaigns separately to see if brand is picking up at all. If so, this is a good story to tell. 
*/ 
SELECT
	YEAR(website_sessions.created_at) AS yr, 
    MONTH(website_sessions.created_at) AS mo, 
...
/*
3.	While we’re on Gsearch, dive into nonbrand, and pull monthly sessions and orders split by device type. 
Want to flex our analytical muscles a little and show the board we really know our traffic sources. 
*/ 
SELECT
	YEAR(website_sessions.created_at) AS yr, 
    MONTH(website_sessions.created_at) AS mo, 
...
SELECT * FROM website_pageviews WHERE website_session_id > 17145;
...
/*
7.	For the landing page test analyzed previously, it would be great to show a full conversion funnel 
from each of the two pages to orders. Use the same time period analyzed last time (Jun 19 – Jul 28).
*/ 
...
/*
8.	Quantify the impact of our billing test. Analyze the lift generated 
